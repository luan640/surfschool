...
export function TripPageContent({ data }: { data: TripData }) {
        ...
        <div className="relative mx-auto grid max-w-7xl gap-8 px-6 py-12 lg:grid-cols-[1.15fr_.85fr] lg:py-16">
          <div>
            ...
            <div className="mt-8 grid gap-3 sm:grid-cols-4">
              <InfoCard icon={<CalendarDays size={16} />} label={t.trip_departure} value={formatDateTime(data.departure_at ?? data.starts_at)} />
              <InfoCard icon={<CalendarDays size={16} />} label={t.trip_arrival} value={formatDateTime(data.arrival_at ?? data.ends_at)} />
              <InfoCard icon={<MapPin size={16} />} label={t.trip_destination} value={data.location ?? t.trip_tbd} />
              <InfoCard icon={<Users size={16} />} label={t.trip_spots} value={data.capacity ? t.trip_spots_count(data.capacity) : t.trip_spots_limited} />
            </div>
          </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-8 lg:grid-cols-[1.1fr_.9fr]">
        <div className="space-y-6">
          <section className="rounded-2xl border border-slate-200 bg-white p-6">
            <div className="mb-3 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">{t.trip_investment}</div>
            <div className="font-condensed text-5xl font-bold text-slate-900">{formatPrice(Number(data.price))}</div>
          </section>

          {data.description && (
            <section className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className="mb-3 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">{t.trip_about}</div>
          ...
          {(data.images?.length ?? 0) > 0 && (
            <section className="rounded-2xl border border-slate-200 bg-white p-6">
              <div className="mb-4 text-[11px] font-bold uppercase tracking-[0.18em] text-slate-400">{t.trip_gallery}</div>
